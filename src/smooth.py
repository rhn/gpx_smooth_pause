...
def time_margins(it, margin):
    ...
    start = it[0]
    start_idx = find_idx(it, lambda point: time_diff(start, point))
    end_idx = len(it) - find_idx(reverse(it), lambda point: time_diff(point, start))
...
def find_stops2(segment, get_uncertainty):
    ...
    for point, future in seg_iter:
        if is_moving(point, future, get_uncertainty) is not False:
            ...
            continue
        stopped_points = list(while_overlap([point] + list(future), [], get_uncertainty))
        yield time_margins(stopped_points, TRIGGER_TIME / 2)
        seg_iter.advance_after(stopped_points[-1])


def replace_stops(segment, stops):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', choices=stop_finders, dafault='good')
    parser.parse_args()
    with open(parser.file) as infile:
        gpx = gpxpy.parse(infile)
    
    output = sys.stdout
    
    stop_finder = stop_finders[parser.method]
    
    get_uncertainty_m = n900_uncertainty_m
    for track in gpx.tracks:
        for segment in track.segments:
            #save_stops(output, stop_finder(segment.points, get_uncertainty_m))
            replace_stops(segment, stop_finder(segment.points, get_uncertainty_m))
